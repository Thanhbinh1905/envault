...
There is no capability token, grant, or agent session in this contract; authorization is same-uid trust in the daemon's peer check, gated by which profiles are in the loaded set (see the glossary).
...
- `envault run --profile <name> -- <command> [args...]` (repeatable, or `--workspace <name>`), run by the human, not by you: it injects plaintext directly into the spawned child's environment and never prints it; every named profile must already be in the loaded set
