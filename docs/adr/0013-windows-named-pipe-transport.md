...
`envault-platform`'s Windows private-path hardening calls into it to restrict access control lists on private files, directories, and the daemon's transport endpoint; its reparse-point rejection and same-file-identity checks (via `GetFileInformationByHandle`) need no `unsafe` code and remain outside this crate.
...
The daemon-side async named-pipe listener and accept loop share `daemon.rs`'s `Server`, connection dispatch (`handle_connection`/`handle_connection_with_timing`), and framing (`read_async_frame`/`write_async_frame`) between Unix and Windows through generic `AsyncRead`/`AsyncWrite` bounds rather than a hardcoded `UnixStream` type, with only the listener type, accept mechanism, and peer-identity resolution differing per platform behind `#[cfg(unix)]`/`#[cfg(windows)]`. The Windows accept loop creates one named-pipe instance per iteration via `envault-windows-ffi`'s tokio-native `create_named_pipe_server`, authenticates the connecting client's PID and security identifier before dispatch, and derives a `PeerIdentity.session_id` from the Windows Terminal Services session (`ProcessIdToSessionId`), the correct analog of the Unix login-session ID `getsid` provides, so rate-limit binding retains the same "per login session of the owning user" granularity on both platforms (the admin lease itself was later rescoped to `uid` alone; see ADR 0009's addendum). This layer is compile-verified via `cargo xwin check --workspace --all-targets`; it is exercised against a real Windows runtime by CI's `windows-runtime` job.

Deferred: the client-side `start` command's automatic spawn-on-demand daemon launch is unimplemented on Windows even though the daemon it would spawn runs. Unix daemonization (`setsid`, descriptor handling, the bootstrap stdio handshake in `spawn_daemon`) has no direct Windows equivalent, and getting Windows process launch and detachment right is a distinct piece of work from the transport; a human or supervisor can still run `envaultd` directly and reach every other client operation. Also deferred: a client-side read/write timeout equivalent to the Unix socket's `set_read_timeout`/`set_write_timeout`, since a `File`-wrapped named-pipe handle has no synchronous timeout primitive in `std` (a watchdog thread or overlapped I/O would be needed); and the narrower Windows shutdown-signal set (`ctrl_c` only, not yet `ctrl_break`/`ctrl_close`/`ctrl_shutdown`), which matters once `envaultd` runs as a service rather than an interactive process. All three are tracked follow-ups, not silently dropped scope.
