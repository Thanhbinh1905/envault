...
The named-pipe security descriptor and process-token SID comparison this ADR requires have no safe published crate that fully covers them, so this ADR grants one narrowly scoped exception: a single Windows-only module, `envault-platform`'s `windows_ffi` submodule, gated `#[cfg(windows)]` and carrying its own `#![allow(unsafe_code)]` at the module boundary, is the sole place in the entire workspace permitted to contain `unsafe` blocks.
Every crate's workspace-inherited `forbid(unsafe_code)` lint remains unchanged; this is a module-local `allow` that overrides the inherited `forbid` for that one module only, exactly as a normal Rust lint override works, not a change to the workspace lint table.
The module depends only on `windows-sys`, the official Microsoft-maintained low-level Win32 binding crate, uses the minimal feature set needed (named pipes, security descriptors, process and token queries), and every `unsafe` block carries a safety comment stating the invariant it relies on (buffer sizes, handle validity, lifetime of borrowed data) per standard Rust unsafe-code documentation convention.
This module exposes only safe function signatures to its callers; no `unsafe` keyword appears anywhere outside it, so every other crate's `forbid(unsafe_code)` remains fully enforced and untouched.
This exception is scoped to Windows peer authentication and named-pipe security descriptor construction specifically; it is not a general license to add unsafe code elsewhere, and any future use of this module for an unrelated purpose should prompt a fresh review of whether it still belongs there.
